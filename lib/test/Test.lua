...
setupDual(TEST, "true", function(v) return v == true end)
setupDual(TEST, "false", function(v) return v == false end)
setupDual(TEST, "truthy", function(v) return v end)
setupDual(TEST, "nil", isnil)
setupDual(TEST, "numeric", isnumeric)
setupDual(TEST, "number", isnumber)
setupDual(TEST, "function", isfunction)
setupDual(TEST, "table", istable)
setupDual(TEST, "string", isstring)
setupDual(TEST, "empty", function(v) return isnil(v) or #v == 0 end)
setupDual(TEST, "non-empty string", function(v) return not isnil(v) and isstring(v) and #v > 0 end)
setupDual(TEST, "nil or string", function(v) return isnil(v) or isstring(v) end)